...
from canopyrs.engine.models.segmenter.train_sam.loss_fns import sam2_loss
...
def build_sam3_model(config: SegmenterConfig):
    """Build SAM3 Tracker model with proper freezing strategy (like SAM2)."""
    MODEL_MAPPING = {
        't': "facebook/sam3",
        's': "facebook/sam3",
        'b': "facebook/sam3",
...
def setup_sam3_trainer(train_dataset_name: str, valid_dataset_name: str, config: SegmenterConfig, model_name: str):
    """Set up SAM3 training components (mirroring SAM2 setup)."""
    ...
    # ✅ FIX: Separate param groups like SAM2
    encoder_params = []
    non_encoder_params = []
    ...
    # ✅ FIX: Warmup + cosine decay like SAM2
...
def train_step(processor, model, batch, optimizer, scaler, device, config, global_step=0, vis_dir=None):
    """Single SAM3 training step (PVS-style, like SAM2 but using tracker)."""
    if len(batch) == 0:
        return None
    ...
    with torch.amp.autocast(device_type='cuda', enabled=use_amp):
        for sample_idx, data in enumerate(batch):
            ...
            # ---- Run SAM3 tracker with GT boxes as prompts (PVS) ----
...
def validate_sam3(processor, model, valid_loader, device, config, epoch, vis_dir=None):
    """Validate SAM3 tracker model (PVS-style, consistent with train_step)."""
    print(f"\nValidating...")
    model.eval()
...
def train_sam3(config: SegmenterConfig):
    """Train SAM3 (mirroring SAM2 training pattern)."""
    print(f"\n{'='*80}")
