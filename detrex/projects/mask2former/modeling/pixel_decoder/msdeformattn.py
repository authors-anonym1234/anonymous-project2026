...
# Use absolute imports so LazyConfig doesn't block relative imports inside non-config modules
...
from ...modeling.transformer_decoder.transformer import _get_clones, _get_activation_fn
...
class MSDeformAttnTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # self attention using detrex MultiScaleDeformableAttention with its own residual/dropout
        src = self.self_attn(
            query=src,
            query_pos=pos,
            value=src,
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            if self.training and self.use_checkpoint:
                output = checkpoint(
                    layer, output, pos, reference_points, spatial_shapes, level_start_index, padding_mask, use_reentrant=True
                )
            else:
